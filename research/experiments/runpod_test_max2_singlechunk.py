...
def main():
    ...
    parser.add_argument("--quality-profile", choices=("standard", "legacy_refined"), default="standard")
    parser.add_argument("--engine", choices=("auto", "diffueraser", "propainter"), default="auto")
    ...
    try:
        ...
        plan = cpu("POST", job, "plan", json={"target_seconds": 15, "overlap": 0, "use_scenes": False})
        ...
        output = run({"chunk_index": 0, "source_url": signed(job, "chunks/0/source"),
            ...
            "options": {"dynamic": True, "key_step": 1, "verify": True, "selective_second_pass": False,
                        "protect_subject": False, "enhance": False, "strategy": "inpaint",
                        "quality_profile": args.quality_profile, "engine": args.engine}}, 600)
        encoded = output.pop("output_b64", None)
        report["result"] = output
        save()
        if output.get("quality_profile") != args.quality_profile:
            raise RuntimeError("Worker did not confirm requested quality profile")
        if not encoded:
