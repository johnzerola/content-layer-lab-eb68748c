"""Run exactly one paid Pedro regression smoke with frozen inputs.
...
EXPECTED_INPUT_SHA256 = "ac3220aa3b4bcf92332a06cb42412477df1f438fbcb22127db17137deb1fe9b9"
...
def main() -> int:
    ...
    if sha256(args.source) != EXPECTED_INPUT_SHA256:
        raise ValueError("Pedro input hash differs from the frozen contract")
    contract = video_contract(probe(args.source))
    if (contract["width"], contract["height"], contract["fps"], contract["frames"]) != (1080, 1920, "30/1", 150):
        raise ValueError(f"Pedro geometry differs from frozen contract: {contract}")
    if abs(contract["duration"] - 5.0) > 0.01:
        raise ValueError("Pedro duration differs from frozen contract")
    regions = json.loads(args.regions.read_text(encoding="utf-8"))
    if len(regions) != 2:
        raise ValueError("exactly two frozen regions are required")
    ...
    report = {
        "schema": "phase6-regression-smoke-v1", "commit": args.commit,
    ...
    try:
        ...
        payload = {
            ...
            "source_is_chunk": True, "start": 0, "end": 5.0, "overlap": 0,
        ...
        missing = set(local_names) - set(report["downloaded_artifacts"])
        if missing:
