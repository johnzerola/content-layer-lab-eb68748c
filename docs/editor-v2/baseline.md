...
- `keptSegments` do V1 normaliza início/fim sem preservar `speed`; o adapter V2 trata a velocidade diretamente sem modificar o fluxo antigo;
...
## Validação da Library V2

- desktop 1440×1000 e viewport estreito 430×932;
- largura estreita permaneceu em 430 px, sem overflow horizontal da página;
- busca local, troca de categoria por teclado, favorito, adicionar por botão, drag/drop, undo e redo exercitados;
- nenhum erro de console durante o fluxo;
- a captura do editor V1 autenticado e o MP4 de referência continuam pendentes, pois esta execução local não utilizou sessão nem mídia privada do usuário.
