...
import { z } from "zod";
import { completeTikTokOAuth } from "@/lib/tiktok-oauth.functions";
...
export const Route = createFileRoute("/integracoes_/tiktok/callback")({
  validateSearch: callbackSearch,
  component: TikTokOAuthCallback,
  head: () => ({
    meta: [
      { title: "Conectando TikTok — VaiViral" },
      { name: "description", content: "Conclusão da autorização da conta do TikTok para publicação automática." },
      { property: "og:title", content: "Conectando TikTok — VaiViral" },
      { property: "og:description", content: "Conclusão da autorização da conta do TikTok." },
...
function TikTokOAuthCallback() {
  ...
  const completeOAuth = useServerFn(completeTikTokOAuth);
  ...
  useEffect(() => {
    ...
    if (search.error || !search.code || !search.state) {
      setResult({
        ok: false,
        message: search.error_description || "A autorização do TikTok foi cancelada.",
      });
      return;
    }
      ...
      .then((response) => {
        setResult(
          response.ok
            ? {
                ok: true,
                message: `Conta conectada: ${response.accounts
      ...
      <section className="w-full max-w-md rounded-2xl border border-border bg-surface p-6 text-center">
        {!result ? (
          <>
            ...
            <h1 className="mt-4 font-display text-xl font-semibold">Conectando TikTok…</h1>
          </>
        ) : (
          <>
            {result.ok ? (
            ...
            <h1 className="mt-4 font-display text-xl font-semibold">
              {result.ok ? "TikTok conectado" : "Conexão não concluída"}
            </h1>
