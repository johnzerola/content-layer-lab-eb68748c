...
import { ImagePlus, Move, Play, RotateCw } from "lucide-react";
  ...
  return (
    <section className="editor-v2-stage relative flex h-full min-h-0 flex-col overflow-hidden" aria-label="Prévia do vídeo">
      <div className="editor-v2-stage-header flex h-10 shrink-0 items-center justify-between px-3">
        ...
        <div aria-live="polite" className={`flex items-center gap-1.5 text-[10px] ${playing ? "font-medium text-primary" : "text-muted-foreground"}`}>{playing ? <Play className="size-3 fill-current" /> : <Move className="size-3" />}<span className="hidden sm:inline">{playing ? "Reproduzindo · prévia limpa" : "Arraste para posicionar"}</span></div>
      </div>
      <div className="editor-v2-canvas-workspace relative grid min-h-0 flex-1 place-items-center overflow-hidden p-4 sm:p-6" onPointerDown={(event) => { if (!playing && event.target === event.currentTarget) onSelect(null); }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; }} onDrop={(event) => { event.preventDefault(); if (event.dataTransfer.files.length) { onImportFiles(event.dataTransfer.files); return; } const mediaId = event.dataTransfer.getData("application/x-vaiviral-media-asset"); if (mediaId) { onDropMediaAsset(mediaId); return; } const id = event.dataTransfer.getData("application/x-vaiviral-library-item"); if (id) onDropLibraryItem(id); }}>
        ...
        <div ref={frameRef} data-testid="editor-v2-canvas" data-playback-mode={playing ? "clean" : "edit"} className={`editor-v2-canvas-frame relative max-h-full max-w-full overflow-hidden rounded-[18px] bg-[#080a0f] ${playing ? "ring-1 ring-primary/20" : ""}`} style={{ aspectRatio: `${project.settings.width}/${project.settings.height}`, height: project.settings.width < project.settings.height ? "min(100%, 620px)" : "auto", width: project.settings.width < project.settings.height ? "auto" : "min(100%, 820px)" }} onPointerDown={(event) => { if (!playing && event.target === event.currentTarget) onSelect(null); }}>
          ...
          {!playing && <div className="pointer-events-none absolute inset-[5%] border border-dashed border-white/10" aria-hidden />}
          {!playing && draft && (Math.abs(draft.transform.x - 50) < 0.01 || Math.abs(draft.transform.y - 50) < 0.01) && <><span className="pointer-events-none absolute inset-y-0 left-1/2 w-px bg-primary/65" /><span className="pointer-events-none absolute inset-x-0 top-1/2 h-px bg-primary/65" /></>}
        </div>
      </div>
    </section>
  );
...
function CanvasClip({ clip, transform, transition, presentation, selected, primary, currentTime, playing, drivesPlaybackClock, onPlaybackTime, assetUrl, caption, onSelect, onMove, onResize, onRotate, onKeyboardTransform }: { clip: Clip; transform: ClipTransform; transition: { opacity: number; translateX: number; scale: number }; presentation: ResolvedClipPresentation; selected: boolean; primary: boolean; currentTime: number; playing: boolean; drivesPlaybackClock: boolean; onPlaybackTime: (time: number) => void; assetUrl?: string; caption?: { cue: CaptionCue; preset: Record<string, unknown> }; onSelect: (additive: boolean) => void; onMove: (event: React.PointerEvent) => void; onResize: (event: React.PointerEvent) => void; onRotate: (event: React.PointerEvent) => void; onKeyboardTransform: (transform: ClipTransform) => void }) {
  ...
  return (
    <div role={playing ? undefined : "button"} tabIndex={playing ? -1 : 0} aria-label={playing ? undefined : `${clip.name}${selected ? ", selecionado" : ""}`} onPointerDown={playing ? undefined : onMove} onKeyDown={(event) => { if (playing) return; if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(event.ctrlKey || event.metaKey || event.shiftKey); return; } const step = event.shiftKey ? 5 : 1; if (event.key === "ArrowLeft") onKeyboardTransform({ ...transform, x: transform.x - step }); else if (event.key === "ArrowRight") onKeyboardTransform({ ...transform, x: transform.x + step }); else if (event.key === "ArrowUp") onKeyboardTransform({ ...transform, y: transform.y - step }); else if (event.key === "ArrowDown") onKeyboardTransform({ ...transform, y: transform.y + step }); else return; event.preventDefault(); event.stopPropagation(); }} className={`absolute touch-none select-none outline-none ${playing ? "pointer-events-none" : ""}`} style={{ left: `${transform.x}%`, top: `${transform.y}%`, width: `${transform.width}%`, height: `${transform.height}%`, opacity: transform.opacity * transition.opacity * presentation.opacity, transform: `translate(calc(-50% + ${transition.translateX + presentation.translateX}%), calc(-50% + ${presentation.translateY}%)) rotate(${transform.rotation + presentation.rotation}deg) scale(${transform.scale * transition.scale * presentation.scale})`, filter: presentation.filter, zIndex: selected && !playing ? 30 : 10 }}>
