...
export function CreativeInspectorV2({ clip, selectionCount = 1, onPatch }: { clip: Clip; selectionCount?: number; onPatch: (patch: Partial<Clip>) => void }) {
  ...
  return <section className="border-b border-white/8 py-4">
    ...
    {tab === "effects" && <EffectStack effects={clip.effects} duration={Number(clip.projectEnd) - Number(clip.projectStart)} onChange={(effects) => onPatch({ effects })} />}
  </section>;
}
...
function EffectStack({ effects, duration, onChange }: { effects: EffectInstance[]; duration: number; onChange: (effects: EffectInstance[]) => void }) {
  ...
  const patchEffect = (id: string, parameters: Record<string, number | string | boolean>) => onChange(effects.map((item) => item.id === id ? { ...item, parameters: { ...item.parameters, ...parameters } } : item));
  return <div className="space-y-2">{effects.map((effect) => { const start = Number(effect.parameters["start"] ?? 0); const end = Number(effect.parameters["end"] ?? duration); return <div key={effect.id} className="rounded-xl border border-white/8 bg-white/[0.025] p-2.5"><div className="flex items-center gap-2"><button type="button" aria-pressed={effect.enabled} onClick={() => onChange(effects.map((item) => item.id === effect.id ? { ...item, enabled: !item.enabled } : item))} className={`h-7 rounded-md px-2 text-[9px] ${effect.enabled ? "bg-primary/18 text-primary" : "bg-white/5 text-muted-foreground"}`}>{effect.enabled ? "Ativo" : "Desligado"}</button><span className="flex-1 text-[10px] font-medium">{effect.definitionId}</span><button type="button" aria-label={`Remover ${effect.definitionId}`} onClick={() => onChange(effects.filter((item) => item.id !== effect.id))} className="text-muted-foreground hover:text-destructive"><Trash2 className="size-3.5" /></button></div><div className="mt-2 grid grid-cols-2 gap-2"><TimeField label="Início" value={start} max={Math.max(0, end - .04)} onChange={(value) => patchEffect(effect.id, { start: Math.max(0, Math.min(end - .04, value)) })} /><TimeField label="Fim" value={end} max={duration} onChange={(value) => patchEffect(effect.id, { end: Math.min(duration, Math.max(start + .04, value)) })} /></div><Range label="Intensidade" value={Number(effect.parameters["intensity"] ?? .6)} min={.1} max={1} step={.05} onChange={(intensity) => patchEffect(effect.id, { intensity })} /></div>; })}</div>;
}

function TimeField({ label, value, max, onChange }: { label: string; value: number; max: number; onChange: (value: number) => void }) { return <label className="text-[9px] text-muted-foreground">{label}<span className="relative mt-1 block"><input type="number" min="0" max={max} step="0.05" value={value.toFixed(2)} onChange={(event) => onChange(Number(event.target.value))} className="h-8 w-full rounded-lg border border-white/10 bg-black/25 px-2 pr-5 text-[10px] tabular-nums text-foreground" /><span className="pointer-events-none absolute right-2 top-2 text-[8px]">s</span></span></label>; }
