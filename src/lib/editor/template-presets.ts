...
export const READY_TEMPLATES: ReadyTemplate[] = [
  {
    ...
    build: (l) => {
      const bar = shape(l, { name: "Barra do hook", x: 6, y: 6, width: 88, height: 14, fill: "#ffd93d", radius: 20, animationIn: anim("slideDown") } as Partial<TemplateLayer>);
  ...
  },
  {
    ...
    build: (l) => {
      ...
      const a = text([...l, left, right], { name: "FATO", text: "FATO", x: 2, y: 3, width: 46, height: 10, fontSize: 74, fontWeight: 900, uppercase: true, animationIn: anim("slideRight") } as Partial<TemplateLayer> & { text: string });
      const b = text([...l, left, right, a], { name: "FAKE", text: "FAKE", x: 52, y: 3, width: 46, height: 10, fontSize: 74, fontWeight: 900, uppercase: true, animationIn: anim("slideLeft") } as Partial<TemplateLayer> & { text: string });
  ...
  },
  {
    ...
    build: (l, id) => {
      ...
      const handle = text([...l, pill], { name: "Handle", text: `@${id.handle}`, x: 26, y: 79, width: 48, height: 5, fontSize: 40, fontWeight: 800, animationIn: anim("fadeIn", 0.4, 0.1) } as Partial<TemplateLayer> & { text: string });
  ...
  },
  {
    ...
    build: (l, id) => {
      const bar = shape(l, { name: "Card do nome", x: 6, y: 70, width: 60, height: 10, fill: "#0b0b12cc", radius: 12, animationIn: anim("slideRight") } as Partial<TemplateLayer>);
      const name = text([...l, bar], { name: "Nome", text: id.name, x: 9, y: 71, width: 54, height: 5, fontSize: 44, fontWeight: 800, align: "left", animationIn: anim("fadeIn", 0.4, 0.15) } as Partial<TemplateLayer> & { text: string });
      const role = text([...l, bar, name], { name: "Cargo", text: "Criador de conteúdo", x: 9, y: 76, width: 54, height: 4, fontSize: 30, fontWeight: 600, align: "left", color: "#4cc9f0", animationIn: anim("fadeIn", 0.4, 0.25) } as Partial<TemplateLayer> & { text: string });
  ...
  },
  {
    ...
    build: (l) => [
      shape(l, { name: "Barra de progresso", x: 0, y: 96, width: 100, height: 2, fill: "#31f39a", radius: 0, animationIn: anim("slideRight", 1) } as Partial<TemplateLayer>),
    ],
  },
  {
    id: "legenda-caixa",
    ...
    build: (l) => {
      const box = shape(l, { name: "Caixa da manchete", x: 6, y: 82, width: 88, height: 12, fill: "#111118e6", radius: 14, animationIn: anim("slideUp") } as Partial<TemplateLayer>);
      const head = text([...l, box], { name: "Manchete", text: "Escreva a manchete aqui", x: 8, y: 84, width: 84, height: 8, fontSize: 42, fontWeight: 800, animationIn: anim("fadeIn", 0.4, 0.1) } as Partial<TemplateLayer> & { text: string });
      return [box, head];
    },
  },
];
