...
export function createEffectInstance(definition: EffectDefinition, instanceId: string) {
  return {
    id: instanceId,
    definitionId: definition.id,
    enabled: true,
    parameters: Object.fromEntries(Object.entries(definition.parameters).map(([key, value]) => [key, value.default])),
  };
}
...
export function validateTemplateDefinition(template: TemplateDefinition): string[] {
  ...
  if (!template.document?.layers?.length) errors.push("document.layers é obrigatório");
  if (!Number.isFinite(template.previewTime) || template.previewTime < 0 || template.previewTime > template.duration) errors.push("previewTime deve estar dentro da duração");
