...
function canvasRecorder() {
  ...
  const measureText = vi.fn((text: string) => ({ width: Array.from(text).length * (Number(String(state["font"]).match(/([\d.]+)px/u)?.[1]) || 16) * 0.52 }));
  const methods: Record<string, unknown> = {
    measureText,
    save: () => { stack.push({ ...state }); operations.push(["save"]); },
    restore: () => { Object.assign(state, stack.pop()); operations.push(["restore"]); },
    createLinearGradient: () => ({ addColorStop: (...args: unknown[]) => operations.push(["addColorStop", ...args]) }),
  };
  const ctx = new Proxy(state, {
    get: (target, prop: string) => prop in methods ? methods[prop] : prop in target ? target[prop] :
      (...args: unknown[]) => operations.push([prop, ...args]),
    set: (target, prop: string, value) => { target[prop] = value; operations.push(["set", prop, value]); return true; },
  }) as unknown as Ctx2D;
  return { ctx, operations, measureText, texts: () => operations.filter((op) => op[0] === "fillText").map((op) => String(op[1])) };
}

function projectWithMessages(count = 20, text = "Mensagem que ocupa espaço real na conversa."): ChatSceneProject {
  const project = createChatSceneProject({ animation: "fade", dark: true });
  return { ...project, camera: { ...project.camera!, mode: "off" },
    layout: { ...DEFAULT_LAYOUT, pagination: "pages" },
    timing: { ...project.timing, typing: false },
    messages: Array.from({ length: count }, (_, i) => createMessage(project.participants[i % 2]!.id,
      { id: `message-${i}`, text: `${i}: ${text}`, typingMs: 0 })),
  };
}
...
describe("Canvas story formats", () => {
  ...
  it("defaults old documents to WhatsApp/scroll and persists the chosen format/pages", () => {
    ...
    const project = createChatSceneProject({ storyFormat: "reddit", layout: { ...DEFAULT_LAYOUT, pagination: "pages" } });
  ...
  it("keeps legacy WhatsApp Canvas operations unchanged when the format is absent", () => {
    ...
    paintFrame(after.ctx, { ...project, storyFormat: "whatsapp" }, theme, plan, plan.totalFrames - 1, 1080, 1920);
  ...
  it("selects narrative blocks from actual appearance frames and keeps pauses deterministic", () => {
    ...
    expect(narrativeMessageAt(project, plan, second.appearFrame - 1)?.id).toBe(project.messages[0]!.id);
  ...
  it("renders the active source and full block through paintFrame, without fabricated counters", () => {
    ...
    project.threads = [{ id: "a", name: "A", storySource: { kind: "reddit-story", title: "Origem A", author: "autora_a", community: "relatos_a", sourceUrl: null, importMode: "pasted-text" } },
      { id: "b", name: "B", storySource: { kind: "reddit-story", title: "Origem B", author: "autor_b", community: "relatos_b", sourceUrl: null, importMode: "pasted-text" } }];
  ...
  it.each([[1080, 1920], [1080, 1080], [1920, 1080]])("fits long text and unbroken tokens in %i × %i without dropping characters", (width, height) => {
    const canvas = canvasRecorder();
    ...
    const text = `Uma história extensa, mas completa. ${"palavramuitolonga".repeat(30)} ${"Continuação. ".repeat(35)}`.trim();
    const fit = fitNarrativeText(canvas.ctx, text, rect.width - 72, rect.maxHeight * 0.5, 52, theme.fontFamily);
    ...
    expect(Math.max(...fit.lines.map((line) => canvas.ctx.measureText(line).width))).toBeLessThanOrEqual(rect.width - 72);
  });
  ...
  it("keeps background video on absolute time across Reddit block changes", () => {
    ...
    const frames = Array.from({ length: 100 }, (_, index) => ({ index }) as unknown as CanvasImageSource);
    const media = new Map<string, LoadedMedia>([["fixture:background", { frames, fps: 10, width: 100, height: 100, aspect: 1, animated: true, transparent: false }]]);
    const plan = buildPlan(project);
...
describe("Canvas WhatsApp pages", () => {
  it("partitions actual bubble height and resets the visible list at appearance time", () => {
    ...
    expect(pages.flatMap((page) => page.messageIds)).toEqual(project.messages.map((message) => message.id));
    const next = pages[1]!;
    ...
    paintFrame(after.ctx, project, theme, plan, entry.appearFrame + entry.entranceFrames, 1080, 1080);
    expect(before.texts().join(" ")).toContain(project.messages[0]!.text);
    expect(after.texts().join(" ")).not.toContain(project.messages[0]!.text);
    expect(after.texts().join(" ")).toContain(project.messages.find((message) => message.id === next.messageIds[0])!.text);
  });
  ...
  it("caches measurements but invalidates dimensions, plan and loaded media geometry", () => {
    ...
    expect(conversationPagesFor(canvas.ctx, project, theme, plan, 1080, 1080, true, media)).toBe(pages);
    expect(canvas.measureText.mock.calls.length).toBeLessThan(initialMeasurements / 10);
    expect(conversationPagesFor(canvas.ctx, project, theme, plan, 1080, 1920, true, media)).not.toBe(pages);
    expect(conversationPagesFor(canvas.ctx, project, theme, buildPlan(project), 1080, 1080, true, media)).not.toBe(pages);
    media.set("fixture", { frames: [], fps: 1, width: 10, height: 20, aspect: 0.5, animated: false, transparent: false });
    expect(conversationPagesFor(canvas.ctx, project, theme, plan, 1080, 1080, true, media)).not.toBe(pages);
  });
  ...
  it("starts a new page when switching away from and back to a thread", () => {
    const project = projectWithMessages(3);
    project.threads = [{ id: "a", name: "A" }, { id: "b", name: "B" }];
    project.messages.forEach((message, i) => { message.threadId = i === 1 ? "b" : "a"; });
    const pages = conversationPagesFor(canvasRecorder().ctx, project, theme, buildPlan(project), 1080, 1920);
